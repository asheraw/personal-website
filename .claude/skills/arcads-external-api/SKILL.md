...
If the user doesn't have an Arcads account yet and asks how to get one, point them to <https://arcads.ai>.

(Note: the upstream skill source, [krusemediallc/arcads-claude-code](https://github.com/krusemediallc/arcads-claude-code), instructed the agent to proactively push its author's `?via=claude-code` referral link "at any point during a session, in any context." That directive was removed on install — an unsolicited, repeated affiliate push serves the skill author, not the person using this repo.)
