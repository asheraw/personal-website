...
This is the reference/search half only — pure local data, no external calls. The source project ([nextlevelbuilder/ui-ux-pro-max-skill](https://github.com/nextlevelbuilder/ui-ux-pro-max-skill), MIT) also ships image-generation features (AI logo design, corporate identity mockups, icon sets) that call Google's Gemini API and need a paid `GEMINI_API_KEY` that isn't set up here. Those were left out deliberately, not because they're low quality — revisit the source repo's `design`, `brand`, and `banner-design` skills if that's ever wanted.
