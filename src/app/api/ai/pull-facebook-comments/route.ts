...
import { normalizeApifyComments, importFacebookComments, type RawApifyComment } from "@/lib/facebookCommentImport";
...
export async function POST(request: NextRequest) {
  ...
  const comments = normalizeApifyComments(items);

  try {
    const { created, matched } = await importFacebookComments({ client: writeClient, postId, comments });
